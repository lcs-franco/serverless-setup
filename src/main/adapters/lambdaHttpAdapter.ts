...
import { ApplicationError } from "@application/errors/app/ApplicationError";
...
export function lambdaHttpAdapter(
  ...
  return async (event: Event): Promise<APIGatewayProxyResultV2> => {
    try {
      ...
      // This "internalId" needs to be the same name off the inject custom attribute on Cognito user pool
      const accountId =
